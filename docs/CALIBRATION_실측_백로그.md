...
#### 1-B 자동화: HEAD 실측 프로브 (2026-08-15 구현, RoboChem 센서구동 이식)
초시계 대신 **반응기 출구 뒤 OPB 위상센서**가 선단 도달을 자동 기록한다. 표준 경로는 그동안
전 구간 개루프여서 예측이 틀려도 로그에 흔적이 없었다(2026-08-14 반응기 암부 0.3 mL 미계상
= 37.5s 조기 전환이 실런에서만 드러난 사건).

- **켜기**: `system_params.head_probe_mode`
  - `"off"` (기본) — 종전과 완전히 동일, 스레드도 안 뜸
  - `"observe"` — 관측·로깅만. **제어 무영향** → 먼저 이 모드로 몇 런 돌린다
  - `"anchor"` — 실측 엣지로 CollectionTimer 클록 재앵커(수집 경계 전체 이동)
- **관련 키**: `head_probe_window_sec`(기본 150, 예상창 ±) / `head_probe_adc_delta`(>0 이면
  유색 시약 ADC 스텝 검출 활성) / `head_probe_sensor_key`(기본 `"collect"`) /
  `sensor_to_outlet_vol_ml`(원장 post 구간분해에서 자동 반영, 0.0382)
- **읽는 법**: 스텝 종료 시 `[HeadProbe] 결산 — 검출 Xs / 예상 Ys / Δ±Zs …` 1줄.
  트레이스에도 `HeadProbe/summary` 로 남는다. **Δ가 여러 런에서 재현되면 그 값을 반영해
  `outlet_switch_delay_sec` 를 고정**하면 anchor 없이도 1-B 가 끝난다.
- **검출기 2종**: 상전이 엣지(가스 분절이 있을 때) / ADC 스텝(유색 시약 선단, 연속 3회 확인).
  ⚠ 무색 시약 + 가스 없는 연속류면 **광학 대비가 없어 검출 불가** — 그때는 미검출 폴백 로그만
  남고 기존 타이밍이 그대로 쓰인다. 염료 런으로 먼저 대비를 확인할 것.
- **안전 계약**(RoboChem `WaitForPhaseChange` 에서 이식): 예상창 밖 엣지 기각 · 창 진입 시
  스테일 엣지 배수 · 일시정지 중 엣지 무시 · 센서 예외/타임아웃은 전부 타이밍 폴백(예외 전파 없음).
- **검증**: `tests/test_head_arrival_probe.py` 24항목 ALL PASS
